    ...
    try:
        ...
        if not lig.endswith(".sdf"):
            ...
            if receptor!=None and not molecule.GetConformer().Is3D():
                ...
                rec=None
            print(f"After file reading: {molecule.GetNumAtoms()} atoms total, {sum([1 for a in molecule.GetAtoms() if a.GetAtomicNum() == 1])} H")
...
def processing_molecules(lig, only_smarts, smarts, rules, index_all, molecule, write_fragments, picture, output_format, output_prefix, workdir, rec, produce_fragment_mol):
    print(f"At processing_molecules start: {molecule.GetNumAtoms()} atoms total, {sum([1 for a in molecule.GetAtoms() if a.GetAtomicNum() == 1])} H")
    try:
        ...
        if len(bonds) > 0:
            print(f"At start of splitting: {molecule.GetNumAtoms()} atoms total, {sum([1 for a in molecule.GetAtoms() if a.GetAtomicNum() == 1])} H")
            ...
            try:
                ...
                tfragments = Chem.GetMolFrags(cutMolecule, asMols=True)
            fragments = list()
            print('molecule was splitted sucsessefuly')
            print(len(tfragments))
            for f in tfragments:
                # replace radicals with implicit hydrogens
                print(len(f.GetAtoms()))
                for a in f.GetAtoms():
                    re = a.GetNumRadicalElectrons()
                    if re != 0:
                        print(f"RadicalElectrons detected on atom {a}")
...
#                        atom.GetNumExplicitHs(),  atom.GetNumRadicalElectrons(), atom.GetFormalCharge())
            print('fragments was sanitized sucsessefuly')
            if picture:
...
def fix_residue_info(mol):
    """
    Проходим по всем атомам водорода, которые могли быть добавлены (или у которых нет нормального ResidueInfo),
    и копируем ResidueInfo от тяжелого атома, к которому этот водород присоединён.
    """
    for atom in mol.GetAtoms():
        # Проверяем, что это водород
        if atom.GetAtomicNum() == 1:
            # Если у нас нет корректного MonomerInfo (ResidueInfo),
            # или он выглядит как "UNL" вместо ожидаемого
            if not atom.HasProp("_PDBResidueInfo"):
                # Найдём первую связь к тяжёлому атому (в идеале их одна)
                bonds = atom.GetBonds()
                if len(bonds) == 1:
                    heavy_atom = bonds[0].GetOtherAtom(atom)
                    # Копируем MonomerInfo с heavy_atom
                    heavy_info = heavy_atom.GetPDBResidueInfo()
                    if heavy_info is not None:
                        # Создаём новый объект для нашего H
                        new_info = Chem.AtomPDBResidueInfo()
                        # Копируем название остатка, номер, цепь и т.д.
                        new_info.SetResidueName(heavy_info.GetResidueName())
                        new_info.SetResidueNumber(heavy_info.GetResidueNumber())
                        new_info.SetChainId(heavy_info.GetChainId())
                        # Можно задать уникальное имя (label) для атома водорода.
                        # Например, " H  " или как-то иначе (RDKit ожидает 4-символьное имя).
                        # Иначе RDKit поставит дефолтное
                        new_info.SetName(" H  ")
